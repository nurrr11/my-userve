    ...
    try {
        ...
        if (data.success && data.events) {
            document.getElementById('eventsList').innerHTML = data.events.map(event => {
                const isFull = (event.Event_Registered || 0) >= event.Event_Slots;
                return `
                <div class="card">
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div>
                            ...
                            <p><strong>FCFS Quota & Registrations:</strong> ${event.Event_Registered || 0} / ${event.Event_Slots} Students</p>
                        </div>
                        <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 5px;">
                            <span class="badge ${isFull ? 'badge-full' : 'badge-open'}" style="background-color: ${isFull ? '#dc3545' : '#28a745'}; color: white; padding: 4px 8px; border-radius: 4px;">
                                ${isFull ? 'FCFS FULL' : 'FCFS OPEN'}
                            </span>
                            <span style="font-size: 11px; background: #007bff; color: white; padding: 2px 6px; border-radius: 4px;">First-Come, First-Served</span>
                        </div>
                    </div>
                    <hr>
...
async function createEvents() { 
    ...
    try {
        ...
        if (response.ok && data.success) {
            alert('Event created successfully with FCFS slot allocation!');
            closeModal();
            loadEvents();
            refreshEventReport();
        } else if (data.isOverlap) {
            // Friendly popup for organizer time-slot collision
            alert('⚠️ ORGANIZER SCHEDULE CONFLICT\n\n' + data.message);
        } else {
            alert('⚠️ ' + (data.message || 'Error creating event.'));
        }
    } catch (error) {
...
async function refreshEventReport() { 
    try {
        ...
        if (data.success && data.reports.length > 0 && reportTable) {
                ...
                <table class="table" style="width: 100%; border-collapse: collapse; margin-top: 10px;">
                    <thead>
                        <tr style="background-color: #f8f9fa; border-bottom: 2px solid #dee2e6;">
                            <th>Event Name</th><th>Date</th><th>FCFS Slots</th><th>Registered</th><th>Present</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${data.reports.map(r => `
            ...
            reportTable.innerHTML = '<p style="color: #821131;">No report data available.</p>';
        }
                    ...
                    <tbody>
                        ${data.volunteers.map(v => `
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary" onclick="updateAttendance(${v.Volunteer_ID}, 'present')">✅ Present</button>
                                    <button class="btn btn-sm btn-danger" onclick="updateAttendance(${v.Volunteer_ID}, 'absent')">❌ Absent</button>
...
async function submitIssueReport() {
    ...
    if (!visibleTextarea || !visibleTextarea.value.trim()) {
        alert('⚠️ Please write details about the issue before submitting.');
        return;
    }
...
async function loadChatHistory(otherUserId) {
    ...
    display.innerHTML = '<p class="text-muted text-center" style="margin: auto; color: #fff8e6;">Loading conversation...</p>';
...
// ============================================
// STUDENT DASHBOARD FEATURES (WITH FCFS LOGIC)
// ============================================
...
// Load Available Events with First-Come First-Served (FCFS) slot calculation & status display
            ...
            list.innerHTML = data.events.map(event => {
                ...
                <div class="card" style="position: relative;">
                    <div style="position: absolute; top: 15px; right: 15px;">
                        <span class="badge" style="background-color: #17a2b8; color: white; padding: 4px 8px; border-radius: 4px; font-size: 11px;">FCFS Basis</span>
                    </div>
                    <h3 style="color: #821131; margin-bottom: 15px; text-align: center;">${event.Event_Name}</h3>
                    <p><strong>Event Organizer:</strong> ${event.Organizer_Name}</p>
                    <p><strong>Event Date:</strong> ${formatDate(event.Event_Date)}</p>
                    <p><strong>Event Time:</strong> ${formatTime(event.Event_Time)}</p>
                    <p><strong>Event Location:</strong> ${event.Event_Location}</p>
                    <p class="${statusColor}"><strong>Slots Remaining: ${remaining > 0 ? remaining : 0} left</strong> (out of ${event.Event_Slots} FCFS slots)</p>
                    <button class="btn btn-primary" onclick="joinEvent(${event.Event_ID})" ${isFull ? 'disabled' : ''}>
                        ${isFull ? 'FULL (FCFS Reached)' : 'Join Event (FCFS)'}
                    </button>
                    <button class="btn btn-secondary" onclick="openChatWithUser('${event.Organizer_ID}', '${escapeQuotes(event.Organizer_Name)}', 'organizer')">
                        Ask Organizer
                    </button>
                </div>
            `;}).join('');
        }
    } catch (error) { console.error('Load available events error:', error); }
}

// Student Join Event with First-Come First-Served (FCFS) Real-Time Validation & Overlap Error Handling
async function joinEvent(eventId) {
    ...
    try {
        ...
        if (data.success) {
            alert('🎉 ' + (data.message || 'Successfully registered on First-Come, First-Served basis!'));
            loadAvailableEvents(); 
        } else if (data.isAlreadyRegistered || (data.message && data.message.includes('already registered'))) {
            alert('⚠️ You are already registered for this event.');
        } else if (data.isOverlap) {
            // Student schedule collision popup
            alert('⚠️ STUDENT SCHEDULE OVERLAP DETECTED\n\n' + data.message);
        } else if (data.isFull || response.status === 400) {
            alert('⚠️ FCFS SLOTS FILLED!\n\n' + (data.message || 'All slots for this event are full.'));
            loadAvailableEvents();
        } else {
            ...
            tableBody.innerHTML = data.history.map(row => {
                ...
                return `
                    <tr>
                        ...
                        <td>Organizer ${row.Organizer_Name}</td>
                        ...
                        <td>
                            ${row.certificate_code ? 
                                ...
                                : `<small class="text-muted">Not Issued Yet</small>`
                            }
                        </td>
                    </tr>
                `;
...
async function loadAdminIssueCentre() {
    ...
    try {
        ...
        if (data.success && data.issues.length > 0) {
            ...
            container.innerHTML = '<p class="text-muted"  style="color: #821131;">No reported issue tickets found.</p>';
        }
    } catch (err) {
